 ...
 */
import { parseFontFamilies, resolveFont, resolveFontStack, isFamilyAvailable } from '../runtime/font-registry.js';
...
function graphemes(text) {
    ...
    const out = [];
    for (const { segment } of _segmenter.segment(text))
        out.push(segment);
    return out;
}
/** True when every code point in `cluster` has a glyph in `variant`. */
function fontCoversCluster(variant, cluster) {
    for (const ch of cluster) {
        if (!fontCovers(variant, ch.codePointAt(0)))
            return false;
    }
    return true;
}
 ...
 * fast path.
 */
export function splitFontRuns(text, fontFamily, weight, style) {
    if (!text)
        return [];
 ...
 */
export function widthOfRuns(doc, runs, opts) {
    let total = 0;
