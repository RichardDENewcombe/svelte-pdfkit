...
export function getLayoutCacheSize() {
...
function hashPageTree(page, width, height) {
 ...
 */
function extractLayoutFlat(node, out) {
    out.push(node.layout ?? null);
    if (node.type !== 'svg') {
        for (const child of node.children)
            extractLayoutFlat(child, out);
    }
}
/**
 ...
 */
function restoreLayoutFlat(node, boxes, idx) {
    const box = boxes[idx.i++];
    node.layout = box ?? undefined;
    if (node.type !== 'svg') {
        for (const child of node.children)
            restoreLayoutFlat(child, boxes, idx);
    }
}
const PAGE_SIZES = {
    A4: [595, 842],
...
function setDim(node, method, value) {
...
function applyStyle(yogaNode, node) {
    const s = node.props.style ?? {};
    if (s.flexDirection != null)
        yogaNode.setFlexDirection(flexDirectionMap[s.flexDirection] ?? FlexDirection.Column);
    if (s.flexGrow != null)
        yogaNode.setFlexGrow(s.flexGrow);
    if (s.flexShrink != null)
        yogaNode.setFlexShrink(s.flexShrink);
    if (s.flexBasis != null)
        yogaNode.setFlexBasis(s.flexBasis);
    if (s.flexWrap != null)
        yogaNode.setFlexWrap(s.flexWrap === 'wrap' ? Wrap.Wrap : Wrap.NoWrap);
    if (s.alignItems != null)
        yogaNode.setAlignItems(alignMap[s.alignItems] ?? Align.Stretch);
    if (s.alignSelf != null)
        yogaNode.setAlignSelf(alignMap[s.alignSelf] ?? Align.Auto);
    if (s.justifyContent != null)
        yogaNode.setJustifyContent(justifyMap[s.justifyContent] ?? Justify.FlexStart);
    if (s.width != null)
        setDim(yogaNode, 'setWidth', s.width);
    if (s.height != null)
        setDim(yogaNode, 'setHeight', s.height);
    if (s.minWidth != null)
        setDim(yogaNode, 'setMinWidth', s.minWidth);
    if (s.maxWidth != null)
        setDim(yogaNode, 'setMaxWidth', s.maxWidth);
    if (s.minHeight != null)
        setDim(yogaNode, 'setMinHeight', s.minHeight);
    if (s.maxHeight != null)
        setDim(yogaNode, 'setMaxHeight', s.maxHeight);
    if (s.gap != null)
        yogaNode.setGap(Gutter.All, s.gap);
    if (s.margin != null)
        yogaNode.setMargin(Edge.All, s.margin);
    if (s.marginTop != null)
        yogaNode.setMargin(Edge.Top, s.marginTop);
    if (s.marginRight != null)
        yogaNode.setMargin(Edge.Right, s.marginRight);
    if (s.marginBottom != null)
        yogaNode.setMargin(Edge.Bottom, s.marginBottom);
    if (s.marginLeft != null)
        yogaNode.setMargin(Edge.Left, s.marginLeft);
    if (s.padding != null)
        yogaNode.setPadding(Edge.All, s.padding);
    if (s.paddingTop != null)
        yogaNode.setPadding(Edge.Top, s.paddingTop);
    if (s.paddingRight != null)
        yogaNode.setPadding(Edge.Right, s.paddingRight);
    if (s.paddingBottom != null)
        yogaNode.setPadding(Edge.Bottom, s.paddingBottom);
    if (s.paddingLeft != null)
        yogaNode.setPadding(Edge.Left, s.paddingLeft);
    ...
    if (s.borderWidth != null)
        yogaNode.setBorder(Edge.All, s.borderWidth);
    if (s.position === 'absolute')
        yogaNode.setPositionType(PositionType.Absolute);
    if (s.top != null)
        yogaNode.setPosition(Edge.Top, s.top);
    if (s.right != null)
        yogaNode.setPosition(Edge.Right, s.right);
    if (s.bottom != null)
        yogaNode.setPosition(Edge.Bottom, s.bottom);
    if (s.left != null)
        yogaNode.setPosition(Edge.Left, s.left);
    if (node.type === 'text') {
...
function extractLayout(node, yogaNode, parentX = 0, parentY = 0) {
