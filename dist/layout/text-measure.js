...
import { splitFontRuns, widthOfRuns } from './font-runs.js';
 ...
 */
export function registerFontOnMeasureDoc(name, buffer) {
 ...
 */
function applyMeasureFont(doc, style) {
 ...
 */
function measureWidth(doc, text, style, opts) {
    const runs = styleRuns(text, style);
    if (runs.length <= 1)
        return doc.widthOfString(text, opts);
 ...
 */
function naturalLineHeight(doc, style, text) {
    if (text) {
        const runs = styleRuns(text, style);
        if (runs.length > 1) {
            let max = 0;
 ...
 */
export function getLineHeight(style, text) {
    const doc = getMeasureDoc();
    applyMeasureFont(doc, style);
 ...
 */
export function wrapLinesMeta(text, style, maxWidth) {
    ...
    const lang = style.hyphenationLang;
 ...
 */
function makeMeasure(doc, style, text) {
    const multi = styleRuns(text, style).length > 1;
    if (!multi)
        return (s) => doc.widthOfString(s);
    return (s) => {
 ...
 */
function breakWordToFit(measure, word, avail, lang) {
 ...
 * fit on a line.
 */
function wrapParagraph(measure, paragraph, maxWidth, hyphenate, lang) {
    ...
    const startLine = (word) => !hyphenate || measure(word) <= maxWidth ? word : flushOverlong(word);
...
export function measureText(text, style, constrainedWidth = 0) {
    if (!text)
