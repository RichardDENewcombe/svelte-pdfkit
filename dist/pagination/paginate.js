 ...
 */
function autoBottomPad(fixedNodes, pageHeight) {
    let needed = 0;
 ...
 */
function findSlotYStart(page, yStart, yEnd, renderedBefore) {
    let top = yStart;
    function walk(node) {
        if (node.props.fixed)
            return;
...
function sliceNode(node, yStart, yEnd, yOffset = 0, nodeProgress = new Map(), renderedBefore = new Set()) {
    ...
    if (node.type === 'text' && typeof node.props.render !== 'function') {
        ...
        if (lineHeight > 0) {
            ...
            const minOrphans = style.orphans ?? 1;
            const minWidows = style.widows ?? 1;
 ...
 */
function getContentBottom(node) {
    let max = 0;
    function walk(n) {
        if (n.layout) {
 ...
 */
function collectForcedBreaks(page) {
    const breaks = new Set();
    function walk(node) {
        if (node.props.fixed)
            return;
        if (node.layout) {
            if (node.props.breakBefore)
                breaks.add(node.layout.y);
            if (node.props.breakAfter)
                breaks.add(node.layout.y + node.layout.height);
        }
        for (const child of node.children)
            walk(child);
    }
 ...
 */
function collectKeepWithNext(page) {
    const pairs = [];
    function walk(node) {
        if (node.props.fixed)
            return;
 ...
 */
function collectNoWrapPairs(page) {
    const pairs = [];
    function walk(node) {
        if (node.props.fixed)
            return;
 ...
 */
function wouldRenderInSlot(node, yStart, yEnd) {
