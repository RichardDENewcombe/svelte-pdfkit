 ...
 */
export function isSvgImage(src, buffer) {
...
export function drawImage(doc, node) {
 ...
 */
function drawSvgImage(doc, src, buffer, x, y, width, height) {
    const opts = { assumePt: true };
    if (width)
        opts.width = width;
    if (height)
        opts.height = height;
    try {
        ...
        console.warn(`svelte-pdfkit: failed to render SVG image "${src}": ${err.message}`);
    }
}
