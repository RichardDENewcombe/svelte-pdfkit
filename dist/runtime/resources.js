 ...
 */
async function fetchRemote(url, kind) {
    const cfg = getRemoteConfig();
    try {
        await assertPublicUrl(url);
    }
    catch (err) {
        warn(`refusing to fetch ${kind} "${url}": ${err.message}`);
        return null;
    }
    let response;
    try {
        ...
        const reason = err.name === 'TimeoutError' ? `timed out after ${cfg.timeoutMs}ms` : err.message;
...
export function getFontBuffer(src) {
    return cacheGet(fontCache, src);
}
export function getImageBuffer(src) {
