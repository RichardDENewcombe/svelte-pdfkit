...
const instances = new Map<string, { hyphenate(word: string): string[] }>();

let defaultLang = 'en-gb';
let customCallback: HyphenationCallback | null = null;
 ...
 */
export function setDefaultHyphenationLang(lang: string): void {
	defaultLang = lang;
}
 ...
 */
export function registerHyphenationCallback(fn: HyphenationCallback | null): void {
	customCallback = fn;
}

/** Lazily constructs (and caches) a Hypher instance for a bundled language. */
function getInstance(lang: string): { hyphenate(word: string): string[] } | null {
 ...
 */
export function hyphenateWord(word: string, lang: string = defaultLang): string[] {
	if (customCallback) {
	...
	if (core.length < MIN_CORE_LENGTH) return [word];

	const instance = getInstance(lang);
	if (!instance) return [word];
