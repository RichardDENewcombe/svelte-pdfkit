...
function anchorKeyOf(node: PDFNode): string | null {
 ...
 */
export function assignAnchorIds(root: PDFNode): void {
	let seq = 0;
 ...
 */
export function buildAnchorIndex(pages: PDFNode[]): Map<string, AnchorEntry> {
	const index = new Map<string, AnchorEntry>();
	const seenIds = new Set<number>();
	const totalPages = pages.length;

	const walk = (nodes: PDFNode[], pageNumber: number) => {
		for (const node of nodes) {
			const key = anchorKeyOf(node);
			if (key !== null) {
				const id = node.props.__anchorId as number | undefined;
				const isPageCopyOfSeenNode = id != null && seenIds.has(id);
				if (!isPageCopyOfSeenNode) {
					if (id != null) seenIds.add(id);
					if (index.has(key)) {
						warn(
							`Duplicate anchor "${key}" — pageOf('${key}') resolves to the first page it appears on (page ${index.get(key)!.pageNumber}).`
						);
					} else {
			...
			if (node.children.length > 0) walk(node.children, pageNumber);
		}
	};

	for (let i = 0; i < pages.length; i++) walk(pages[i].children, i + 1);
	return index;
}
