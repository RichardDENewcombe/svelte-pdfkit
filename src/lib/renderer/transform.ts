...
const TRANSFORM_KEYS: (keyof StyleProps)[] = [
	'rotate',
	'scale',
	'scaleX',
	'scaleY',
...
export function hasTransform(style: StyleProps): boolean {
...
function resolveToken(token: string, length: number): number {
	switch (token) {
		case 'left':
		case 'top':
			return 0;
...
export function resolveTransformOrigin(
	...
	let yToken: string | undefined;
	const free: string[] = [];
	for (const t of tokens) {
		if (t === 'left' || t === 'right') xToken = t;
		else if (t === 'top' || t === 'bottom') yToken = t;
		else free.push(t);
	}
	for (const t of free) {
		if (xToken === undefined) xToken = t;
		else if (yToken === undefined) yToken = t;
	}
 ...
 */
export function buildTransformMatrix(style: StyleProps, box: LayoutBox): Matrix {
	if (!hasTransform(style)) return IDENTITY;
