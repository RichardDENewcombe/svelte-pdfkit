...
const config: RemoteResourceConfig = {
	timeoutMs: 10_000,
 ...
 * });
 */
export function configureRemoteResources(opts: Partial<RemoteResourceConfig>): void {
	Object.assign(config, opts);
}

/** Returns the current remote-resource config (read-only snapshot semantics). */
export function getRemoteConfig(): Readonly<RemoteResourceConfig> {
	return config;
}
...
export async function assertPublicUrl(rawUrl: string): Promise<void> {
	...
	if (config.allowPrivateHosts || config.allowHost?.(hostname)) return;
...
export function isBlockedAddress(address: string): boolean {
	...
	if (isIP(address) === 4) return isBlockedV4(address);
