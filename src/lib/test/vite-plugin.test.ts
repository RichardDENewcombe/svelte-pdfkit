...
describe('sveltePDF plugin – load hook', () => {
	...
	it('imports the svelte-pdf runtime modules', () => {
		...
		const { code } = loadSimple();
		expect(code).toContain("from 'svelte-pdf/runtime/document.js'");
		expect(code).toContain("from 'svelte-pdf/runtime/resources.js'");
		expect(code).toContain("from 'svelte-pdf/layout/layout.js'");
		expect(code).toContain("from 'svelte-pdf/pagination/paginate.js'");
		expect(code).toContain("from 'svelte-pdf/renderer/render.js'");
	});
...
describe('sveltePDF plugin – .d.ts emission', () => {
	...
	it('the emitted .d.ts includes the AUTO-GENERATED header', () => {
		loadSimpleWith(plugin);
		const content = readFileSync(DTS_PATH, 'utf-8');
		expect(content).toContain('AUTO-GENERATED by svelte-pdf');
	});
